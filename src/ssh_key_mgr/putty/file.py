...
from ssh_key_mgr.putty.encryption.base import DecryptionParams
...
class PuttyFile:
    ...
    @abc.abstractmethod
    def decrypt(self, passphrase: SecretBytes | None) -> PuttyKeyPair:
        raise NotImplementedError()
...
@dataclasses.dataclass(frozen=True, slots=True, kw_only=True)
class PuttyFileV3(PuttyFile, EncryptedFile):
    ...
    @classmethod
    def encrypt(cls, key: PuttyKeyPair, encryption: Encryption, passphrase: SecretBytes | None) -> Self:
        encryption_params = encryption.generate_params()

        private_lines, decryption_params, mac_key, private_padded_wire = encryption_params.encrypt(
            key.private, passphrase
        )
...
def loads(data: bytes) -> PuttyFileV3:
    return ppk.unmarshal(PuttyFileV3, data)


def load(fp: io.Reader[str] | io.Reader[bytes]) -> PuttyFileV3:
    data = fp.read()
...
def dumps(file: PuttyFile) -> bytes:
    stream = ppk.StreamWriter()
    file.marshal_ppk(stream)
    return stream.getvalue()


def dump(file: PuttyFile, fp: io.Writer[bytes]) -> None:
    data = dumps(file)
    fp.write(data)
